...
managerSchema.methods.correctPassword = async function (candidatePassword, currentPassword) {
  return await bcrypt.compare(candidatePassword, currentPassword);
};
managerSchema.methods.changedPasswordAfter = function (JWTTimestamps) {
  if (this.ChangesAt) {
...
managerSchema.methods.createResetPassToken = function () {
